    ...
    return (
        <div className="form">
            <form className="form__inputs" onSubmit={handleSubmit((data) => console.log(data))}>
                <Input watch={watch} validate={isValidFullName} reg={register} name={"FullName"} type={"text"} icon={<AiOutlineUser />} placeholder="Full Name" />
                <Input watch={watch} validate={isValidEmail} reg={register} name={"Email"} type={"text"} icon={<AiOutlineMail />} placeholder="Email Address" />
                {errorCodeFirebase === 'auth/email-already-in-use' && <FormError text={"Email is already registered"} />}
                <Input watch={watch} validate={isValidPassword} reg={register} name={"Password"} type={"password"} icon={<AiOutlineLock />} placeholder="Password" />
                <div className="form__subtext">Date of birth:</div>
                <div className="form__date">
                    <Select reg={register} name={"BirthMonth"} placeholder={"Month"} array={monthsArray} />
                    <Input<number> watch={watch} validate={isValidDay} reg={register} name={"BirthDay"} type="number" maxLength={2} placeholder="Day" />
                    <Input<number> watch={watch} validate={isValidYear} reg={register} name={"BirthYear"} type="number" maxLength={4} placeholder="Year" />
                </div>
