...
export function mainValidator({ wEmail, wFullName, wPassword, wBirthDay, wBirthYear }: IMainValidation) {
    if (isValidEmail(wEmail) && isValidFullName(wFullName) && isValidPassword(wPassword) && isValidDay(wBirthDay) && isValidYear(wBirthYear)) {
        return true
    } else {
        return false
    }
